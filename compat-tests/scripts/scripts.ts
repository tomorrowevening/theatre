...
const isTheatreDependency = (pkg) =>
  pkg.startsWith('@tomorrowevening/theatre-') || pkg === 'theatric'
...
export async function installFixtures(): Promise<void> {
  ...
  console.log('Releasing @tomorrowevening/theatre-* packages to verdaccio')
 ...
 * dependencies on `@tomorrowevening/theatre-*` packatges to `version`.
 *
 * @param {string} pathToPackageJson absolute path to the package.json file
 * @param {string} version The version to set all `@tomorrowevening/theatre-*` dependencies to
 */
async function patchTheatreDependencies(pathToPackageJson, version) {
  ...
  // find all dependencies on '@tomorrowevening/theatre-*' packages and replace them with the local version
...
async function patchTestPackageJsons(): Promise<() => void> {
  ...
  // replace all dependencies on @tomorrowevening/theatre-* packages with the local version
  for (const pathToPackageJson of packagePaths) {
    patchTheatreDependencies(pathToPackageJson, tempVersion)
  }

  return () => {
    // replace all dependencies on @tomorrowevening/theatre-* packages with the 0.0.1-COMPAT.1
    for (const pathToPackageJson of packagePaths) {
...
/**
 * Assigns a new version to each of @tomorrowevening/theatre-* packages. If there a package depends on another package in this monorepo,
...
/**
 * Builds all the @tomorrowevening/theatre-* packages with version number 0.0.1-COMPAT.1 and publishes
